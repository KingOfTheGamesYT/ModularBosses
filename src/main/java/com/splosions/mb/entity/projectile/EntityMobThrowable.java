 ...
 * 
 */
package com.splosions.mb.entity.projectile;
...
public abstract class EntityMobThrowable extends EntityThrowable
{
	...
	public EntityMobThrowable(World world, EntityLivingBase shooter, EntityLivingBase target, float velocity, float wobble, float FrontToBack, float YOffset, float SideToSide, float HitBoxSize1, float HitBoxSize2) {
		...
		if (d3 >= 1.0E-7D) {
            ...
            shoot(d0 - xOff, d1 + (double) f4 + YOffset, d2 - zOff, velocity, wobble);//0 used to be "d1 + (double) f4" 0 causes the projectile to always fire straight out of entity with no upward or downward arc
		}
	}
	...
	public EntityMobThrowable(World world, EntityLivingBase shooter, EntityLivingBase target, float velocity, float wobble, float MotionY, float YawOffset) {
		...
		if (d3 >= 1.0E-7D) {
			...
			float f4 = (float) d3 * 0.2F;
            shoot(d0 * -1, 0, d2 * -1, velocity, wobble);//0 used to be "d1 + (double) f4" 0 causes the projectile to always fire straight out of entity with no upward or downward arc
	...
	public EntityMobThrowable(World world, EntityLivingBase shooter, BlockPos pos, float velocity, float MotionY) {
		...
		if (d3 >= 1.0E-7D) {
			...
			setLocationAndAngles(shooter.posX - d4, this.posY, shooter.posZ - d5, f2, f3); 
			float f4 = (float) d3 * 0.2F;
            shoot(d0, 0, d2, velocity, 0);//0 used to be "d1 + (double) f4" 0 causes the projectile to always fire straight out of entity with no upward or downward arc
